...
class CacheService:
    ...
    def __init__(self):
        try:
            ...
            # 🛡️ Initialization is synchronous in __init__, 
            # ping() is only for initial health check (v16.4.5).
            # We don't await here, the connection is lazy-initialized.
            logger.info(f"CacheService: Redis Async Handle Created [Prefix: {self.VERSION_PREFIX}]")
    ...
    async def get(self, key: str) -> Optional[Any]:
        ...
        try:
            data = await self.redis.get(v_key)
    ...
    async def set(self, key: str, value: Any, ttl_seconds: int = 600):
        ...
        v_key = self._get_v_key(key)
        try:
            await self.redis.set(v_key, json.dumps(value), ex=ttl_seconds)
        except Exception as e:
            logger.error(f"Cache Set Error [{v_key}]: {e}")

    async def set_nx(self, key: str, value: Any, ttl_seconds: int = 600) -> bool:
        """Atomic Set-if-not-exists with TTL (v15.2.0). 
        Returns True if set, False if already exists."""
        if not self.redis: return False
        v_key = self._get_v_key(key)
        try:
            return bool(await self.redis.set(v_key, json.dumps(value), ex=ttl_seconds, nx=True))
        except Exception as e:
